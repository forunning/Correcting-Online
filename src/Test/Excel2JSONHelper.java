...
public class Excel2JSONHelper {
  ...
  private Object getCellValue(Row row,int cellIndex,FormulaEvaluator formula){
    ...
    return cell.getRichStringCellValue().getString();
		/*
		 * if(cell != null){ switch (cell.getCellType()) { //String类型 case
		 * Cell.CELL_TYPE_STRING: return cell.getRichStringCellValue().getString();
		 * 
		 * //number类型 case Cell.CELL_TYPE_NUMERIC: if
		 * (DateUtil.isCellDateFormatted(cell)) { return
		 * cell.getDateCellValue().getTime(); } else { return
		 * cell.getNumericCellValue(); } //boolean类型 case Cell.CELL_TYPE_BOOLEAN: return
		 * cell.getBooleanCellValue(); //公式 case Cell.CELL_TYPE_FORMULA: return
		 * formula.evaluate(cell).getNumberValue(); default: return null; } }
		 */
  }
