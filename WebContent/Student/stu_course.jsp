...
<body>
	<div id="wrap">
		...
		<div id="jishu">
			<a href="stu_course.jsp?page=1">我的课程</a>
		</div>
		<div id="test-con">
...
$(document).ready(
		...
		var data={stu_id:"<%=session.getAttribute("user")%>"};
		/* var data={stu_id:"1611205119"}; */

		$.ajax({
			url:'/Test/Student/getCourse',
			type:'post',
			data:data,
			dataType:"json",
			success:function(data){
				...
				if(page<=1){
					$(".pagination").append("<li><a href='"+"stu_course.jsp?page="+"1"+"'>«</a></li>");
				}else{
					$(".pagination").append("<li><a href='"+"stu_course.jsp?page="+(page-1)+"'>«</a></li>");
				}
					
				for(var i=0;i<count;i++){
					if((i+1)!=page){
						$(".pagination").append("<li><a href='"+"stu_course.jsp?page="+(i+1)+"'>"+(i+1)+"</a></li>");	
					}
					else{
						$(".pagination").append("<li><a class='active' href='"+"stu_course.jsp?page="+(i+1)+"'>"+(i+1)+"</a></li>");
					}
				}
				if(page>=count){
					$(".pagination").append("<li><a href='"+"stu_course.jsp?page="+(page)+"'>»</a></li>");
				}else{
					$(".pagination").append("<li><a href='"+"stu_course.jsp?page="+(page+1)+"'>»</a></li>");
				}
				...
				for (var i = (page - 1) * 4; i < 4 * page; i++) {
					...
					$("#" + data[i].cou_id).find("a").attr(
							"href",
							"stu_task.jsp?page=1&cou_id=" + data[i].cou_id);
